...
\begin{abstract}
We present a Lean~4/Mathlib formalization of the decision-theoretic infrastructure behind \emph{Classical Lottery in Action} and of the Wakker--Debreu--Koopmans representation layer used by that development.  The artifact has two complementary components.  The file \code{ClassicalLotteryInAction.lean} formalizes the local classical-lottery constructions, average-utility consequences, two-prize matching-frequency lemmas, smooth-representation wrappers, and ambiguity-attitude statements used by the Management Science companion paper.  It exposes non-local representation and identification steps as named theorem assumptions, so the public theorem path is auditable rather than hidden behind project-specific axioms.  The separate Wakker/Debreu--Koopmans development formalizes, to our knowledge for the first time in any proof assistant, Wakker's Theorem~IV.2.7 and the Debreu--Koopmans hard direction, with sorry-free \emph{conditional} wrapper theorems: the public \code{wakker\_IV\_2\_7} consumes the additive-representation construction as an explicit, clearly-labelled hypothesis.  The deep forward construction from the bare structural axioms is \emph{not} claimed --- and is machine-checked \emph{irreducible} from them: it is reduced to a single proven-necessary structural input (the cross-pair Thomsen / double-cancellation hexagon), with both links of the \S IV.5 construction assembled end-to-end around that input, seven machine-checked irreducibility findings, and a sound cardinal-grid companion that discharges the same conclusion once a coordinate scale is given.  The resulting Lean files are sorry-free, split into reviewer-sized modules, and kernel-axiom-clean: the public regression files print only \foundations{} for the audited theorem surfaces, and no \code{\_from\_raw\_axioms} axiom remains anywhere in the development.
\end{abstract}
